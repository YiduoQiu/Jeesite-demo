...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/yhpczd/czzYhpcXgzd/form?id=${czzYhpcXgzd.id}">制度<shiro:hasPermission name="yhpczd:czzYhpcXgzd:edit">${not empty czzYhpcXgzd.id?'查看':'添加'}</shiro:hasPermission><shiro:lacksPermission name="yhpczd:czzYhpcXgzd:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<div id="view" class="form-horizontal">
		<div id="pagecontent">	
			<div class="control-group">
				<label class="control-label">标题：</label>
				<div style="padding-top: 3px;">${czzYhpcXgzd.title}</div>
			</div>
			<div class="control-group">
				<label class="control-label">内容：</label>
				<div style="margin-left:160px;margin-right:10px;padding-top: 3px;">${czzYhpcXgzd.content}</div>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="yhpczd:czzYhpcXgzd:edit"><input id="btnSubmit" class="btn btn-primary" type="button" onclick="switch_content()" value="修 改"/>&nbsp;</shiro:hasPermission>
			<input id="btnCancel" class="btn" type="button" value="返 回" onclick="history.go(-1)"/>
			<a class="btn jquery-word-export" href="javascript:void(0)">导出为word文档</a>
		</div>
	</div>
	<form:form id="inputForm" modelAttribute="czzYhpcXgzd" action="${ctx}/yhpczd/czzYhpcXgzd/save" method="post" class="form-horizontal" style="display:none;">
		<form:hidden path="id"/>
	...
	<script type="text/javascript">
		function switch_content(){
			$("#view").hide();
			$("#inputForm").show();
			$(".nav .active a").html("事故隐患排查治理制度修改");
		}
	</script>
</body>
</html>