...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/aqgl_zrz/czzAqglZrz/form?id=${czzAqglZrz.id}">安全管理责任制<shiro:hasPermission name="aqgl_zrz:czzAqglZrz:edit">${not empty czzAqglZrz.id?'查看':'添加'}</shiro:hasPermission><shiro:lacksPermission name="aqgl_zrz:czzAqglZrz:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<div id="view" class="form-horizontal">
		<div id="pagecontent">	
			<div class="control-group">
				<label class="control-label">标题：</label>
				<div style="padding-top: 3px;">${czzAqglZrz.title}</div>
			</div>
			<div class="control-group">
				<label class="control-label">内容：</label>
				<div style="margin-left:160px;margin-right:10px;padding-top: 3px;">${czzAqglZrz.content}</div>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="aqgl_zrz:czzAqglZrz:edit"><input id="btnSubmit" class="btn btn-primary" type="button" onclick="switch_content()" value="修 改"/>&nbsp;</shiro:hasPermission>
			<input id="btnCancel" class="btn" type="button" value="返 回" onclick="history.go(-1)"/>
			<a class="btn jquery-word-export" href="javascript:void(0)">导出为word文档</a>
		</div>
	</div>
	<form:form id="inputForm" modelAttribute="czzAqglZrz" action="${ctx}/aqgl_zrz/czzAqglZrz/save" method="post" class="form-horizontal" style="display:none">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<a href="${ctx}/aqgl_zrz/czzAqglZrz/help" target="_blank">安全管理责任制模板</a>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">内容：</label>
