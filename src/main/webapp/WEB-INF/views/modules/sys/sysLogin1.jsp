...
<head>
	...
	<style type="text/css">
    	...
    	.form-signin .input-label{cursor:auto;width:210px;height:38px;line-height:39px;text-align:left;position:absolute;margin-top:10px;margin-left:28px;color:#999;}
    	...
    	#blank {height:20%;}
    	#login_box{width:313px;height:313px;position:absolute;top:57%;margin-top:-170px;left:50%;margin-left:-150px;}
    	.layer_box{background:url("../images/login_box.png") center center no-repeat;opacity: 0.9;filter:alpha(opacity=90);width:313px;height:313px;position:absolute;top:57%;margin-top:-170px;left:50%;margin-left:-150px;}
		...
		.name_pwd{width:240px;height:145px;margin:0 auto;margin-top:0px;}
	    ...
	    #navi{height:32%;}
	    /*** Logo ***/
		.logo{text-align:left;width:16%;height:100px;position:relative;left:13%;top:60%;}
		.logo img{width:234px;height:65px;background-color:#fff;border-radius:10px;}
    </style>
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(".name_pwd input").blur(function(){
				if($(this).val()==""){
					$(this).prev().html($(this).prev().attr("default_txt"));
				}
			});
			$(".input-label .mid").focus(function(){
				$(this).prev().html("");
			});
			$(".input-label .mid").blur(function(){
				if($(this).val()==""){
					$(this).prev().html($(this).prev().attr("default_txt"));
				}
			});
...
<body>
	...
	<div id="login_box">
		...
		<!-- <form id="loginForm" action="${ctx}/login" method="post"> -->
			...
			<div class="name_pwd">
				<label class="input-label" for="username" default_txt="请输入用户名">请输入用户名</label>
				<input type="text" id="username" name="username" class="input-block-level required" value="${username}">
				<img class="head" src="../images/login_name.png" />
				<label class="input-label" for="password" default_txt="请输入密码">请输入密码</label>
				<input type="password" id="password" name="password" class="input-block-level required">
				<img class="pwd" src="../images/login_pwd.png" />
	...
	<div class="footer">
		Copyright &copy; ${fns:getConfig('copyrightYear')} <a href="${pageContext.request.contextPath}${fns:getFrontPath()}">${fns:getConfig('productName')}</a> - Powered By 四川创之智 ${fns:getConfig('version')} 
	</div>
